...
# these 2 r still python functions, we want to covert them to tool to use it, langchain did it with @tool, so to covert them into tool so LLM can digest.
# refer to ollama documentation for how to format tools for Ollama. It's a bit more manual than LangChain's @tool, but gives you full control over the tool schema you expose to the LLM.
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        observation = tool_to_use(**tool_args) # invoke() is langchain's wrapper around a function call that also handles tracing, logging, and other features while **tool_args is a direct python function call with none of that.
